...
@section('header_title')
    <div>
        ...
        <div class="text-xs text-tm">
            Update vehicle details in the fleet
        </div>
    </div>
@endsection
...
@section('content')
    <main class="flex-1 overflow-y-auto p-4 lg:p-6">
        ...
        <div class="flex items-start justify-between mb-6">
            <div>
                <div class="flex items-center gap-1.5 text-[11px] text-slate-500 mb-2">
                    ...
                    <i class="fa-solid fa-chevron-right text-[8px]"></i>
                    <span class="text-slate-400">Edit Car</span>
                </div>
                <h1 class="font-display font-bold text-xl text-white">
                    Edit Car
                </h1>
                <p class="text-xs text-slate-500 mt-0.5">
                    Update the details for <span class="text-slate-300 font-semibold">{{ $car->name }}</span>
                </p>
            </div>
        ...
        <form class="max-w-2xl" method="POST" action="{{ route('admin.car.update', $car->id) }}">
            @csrf
            @method('PUT')
                ...
                <div class="px-6 py-5 border-b border-[#1e3040]">
                    ...
                    <div class="flex items-center gap-5">
                        <div class="car-icon-wrap">
                            <i class="fa-solid {{ old('type', $car->type) === 'hiace' ? 'fa-van-shuttle' : 'fa-bus' }}"
                                id="car-type-icon"></i>
                        </div>
                        <div>
                ...
                <div class="px-6 pt-5 pb-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col gap-1.5 sm:col-span-2">
                            ...
                            <input class="form-input" type="text" name="name" value="{{ old('name', $car->name) }}"
                        ...
                        <div class="flex flex-col gap-1.5 sm:col-span-2">
                            ...
                            <input class="form-input" type="text" name="number_plate"
                        ...
                        <div class="flex flex-col gap-1.5">
                            ...
                            <select class="form-select" name="type" id="type-select"
                                ...
                                <option {{ old('type', $car->type) == 'bus' ? 'selected' : '' }} value="bus">Bus
                                </option>
                                <option {{ old('type', $car->type) == 'hiace' ? 'selected' : '' }} value="hiace">Hiace
                                </option>
                            </select>
                            @error('type')
                                <p class="error-text">{{ $message }}</p>
                        ...
                        <div class="flex flex-col gap-1.5">
                            ...
                            <input class="form-input" type="number" name="capacity"
                                value="{{ old('capacity', $car->capacity) }}" placeholder="e.g. 40" min="1" />
                            @error('capacity')
                                <p class="error-text">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex flex-col gap-1.5 sm:col-span-2">
                            ...
                            <select class="form-select" name="user_id">
                                ...
                                @foreach ($drivers as $driver)
                                    <option {{ old('user_id', $car->user_id) == $driver->id ? 'selected' : '' }}
                                        value="{{ $driver->id }}">
                                        {{ $driver->name }}
                                        @if ($driver->phone)
                                            — {{ $driver->phone }}
                        ...
                        <div class="flex flex-col gap-1.5 sm:col-span-2">
                            ...
                            <div class="flex items-center gap-3">
                                <!-- Active -->
                                <label class="status-card {{ old('status', $car->status) == '1' ? 'active' : '' }}"
                                    id="status-active-card">
                                    <input type="radio" name="status" value="1"
                                        {{ old('status', $car->status) == '1' ? 'checked' : '' }} class="hidden"
                                ...
                                <!-- Inactive -->
                                <label class="status-card {{ old('status', $car->status) == '0' ? 'active' : '' }}"
                                    id="status-inactive-card">
                                    <input type="radio" name="status" value="0"
                                        {{ old('status', $car->status) == '0' ? 'checked' : '' }} class="hidden"
                                        onchange="highlightStatus()" />
                    ...
                    <div class="flex items-center justify-end gap-3 pt-5 mt-5 border-t border-[#1e3040]">
                        ...
                        <button type="submit"
                            ...
                            <i class="fa-solid fa-floppy-disk"></i> Update Car
                        </button>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function updateTypePreview(val) {
            ...
            if (cfg) {
                ...
                badgeEl.innerHTML = `<i class="fa-solid fa-circle text-[6px]"></i> ${cfg.label}`;
            } else {
        ...
        // Init on page load — pre-select existing values
